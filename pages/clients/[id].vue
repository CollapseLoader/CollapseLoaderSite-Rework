...
import { Play, Download, ArrowLeft, ShieldCheck, Github, Star } from 'lucide-vue-next'
...
<template>
    <div v-if="client" class="bg-base-200 min-h-screen selection:bg-primary/30 pb-20 overflow-x-hidden">
        <div class="relative min-h-[50vh] flex items-end overflow-hidden pb-12">
            <div class="absolute inset-0 z-0">
                <div class="absolute inset-0 bg-base-300"></div>
                <div class="absolute inset-0 bg-gradient-to-t from-base-200 via-base-200/40 to-transparent z-10"></div>

                <div
                    class="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2 opacity-60">
                </div>
                <div
                    class="absolute bottom-0 left-0 w-[400px] h-[400px] bg-secondary/10 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/4 opacity-40">
                </div>
            ...
            <div class="container mx-auto px-6 relative z-20">
                <NuxtLink :to="localePath('/clients')"
                    ...
                    <span class="text-sm font-bold tracking-wide">{{ t('clients.back_to_library') }}</span>
                </NuxtLink>
                ...
                <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                    <div class="space-y-6 max-w-3xl">
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <span v-if="clientInfo?.working"
                                class="badge badge-success badge-outline gap-1 px-3 py-1 text-[10px] font-bold">
                                <div class="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></div> WORKING
                            </span>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-6">
                            <div
                                ...
                                <span class="font-bold text-white/90">{{ client?.version }}</span>
                            </div>
                            <div class="text-white/40 font-mono text-sm tracking-widest uppercase">ID: {{ client?.id }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div v-for="stat in [
                            { l: t('clients.detail.stats.downloads'), v: clientInfo?.downloads, i: Download, color: 'text-blue-400', bg: 'bg-blue-400/10' },
                            { l: t('clients.detail.stats.version'), v: clientInfo?.version, i: ShieldCheck, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
                            { l: t('clients.detail.stats.status'), v: clientInfo?.working ? t('clients.detail.status_ok') : t('clients.detail.status_error'), i: ShieldCheck, color: clientInfo?.working ? 'text-green-400' : 'text-red-400', bg: clientInfo?.working ? 'bg-green-400/10' : 'bg-red-400/10' },
                        ]" :key="stat.l"
                            ...
                            <div>
                                <div class="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-1">{{
                                    stat.l }}</div>
                                <div class="text-xl font-bold tracking-tight text-white/90">{{ stat.v }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
                    <div
                        ...
                        <div class="relative z-10 space-y-8">
                            <div class="text-center space-y-2">
                                <h3 class="text-3xl font-black">{{ t('clients.detail.launch') }}</h3>
                                <p class="text-sm text-white/40">Ready to play? Launch directly via CollapseLoader.</p>
                            </div>
                            ...
                            <div class="pt-6 border-t border-white/5 flex flex-col gap-4">
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span class="font-bold">Windows / Linux</span>
                                </div>
                                <div class="flex items-center justify-between text-sm">
                                    <span class="text-white/40">Compatibility</span>
                                    <span class="font-bold flex items-center gap-2 text-success">
                                        <ShieldCheck class="w-4 h-4" /> Fully Verified
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="glass-card p-6 rounded-3xl border border-white/5 text-sm space-y-4">
                        <div class="flex items-center gap-3 text-white/60">
                            <Github class="w-5 h-5" />
                            <span>Support the developers by giving CollapseLoader a star.</span>
                        </div>
                        <a href="https://github.com/dest4590/CollapseLoader" target="_blank" class="btn ">
                            <Star class="w-4 h-4" />
                            Star
                        </a>
                    </div>
