...
<template>
    <div class="grid md:grid-cols-2 gap-20 items-center max-w-7xl mx-auto" :class="{ 'md:grid-flow-dense': reverse }">
        <div class="feature-image-container animate-on-scroll group" :class="[
            reverse ? 'anim-fade-right md:col-start-2' : 'anim-fade-left',
        ]" :style="{ '--delay': `${delay}ms` }">
            <div class="feature-icon-wrapper">
                <component :is="icon" class="h-32 w-32 transition-all duration-500 group-hover:scale-110"
                    :class="`text-${iconColor}`" />
            </div>
        </div>

        <div class="text-center space-y-6 animate-on-scroll" :class="[
            reverse
            ...
            <div class="flex flex-wrap gap-3" :class="[
                ...
                <span v-for="badge in badges" :key="badge" class="badge badge-lg" :class="`badge-${iconColor}`">
                    {{ badge }}
                </span>
            </div>
        </div>
